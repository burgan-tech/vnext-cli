...
class SchemaManager {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      ...configDefaults,
      schemaPackageName: process.env.AMORPHIE_SCHEMA_PACKAGE || configDefaults.schemaPackageName || '@amorphie/schema-definitions',
  ...
  getSystemCacheDir() {
    ...
    switch (platform) {
      case 'darwin': // macOS
        return path.join(homedir, 'Library', 'Caches', 'amorphie-cli', 'schemas');
      
      case 'win32': // Windows
        return path.join(process.env.APPDATA || path.join(homedir, 'AppData', 'Roaming'), 'amorphie-cli', 'cache', 'schemas');
      
      case 'linux': // Linux
      default: // Other Unix-like systems
        return path.join(process.env.XDG_CACHE_HOME || path.join(homedir, '.cache'), 'amorphie-cli', 'schemas');
    }
  }
