...
class RefResolver {
  ...
  async validateReferenceConsistency(ref, content, parsedRef) {
    // Skip filename and version validation if content doesn't have version
    // This supports the new ref format without version requirement
    if (!content.version) {
      console.log(chalk.gray(`  ℹ️  Skipping version validation for ref without version: ${ref}`));
      return;
    }
