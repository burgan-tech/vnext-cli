...
class RefResolver {
  constructor(options = {}) {
    this.options = {
      cacheDir: path.join(process.cwd(), '.amorphie-cache'),
      strictMode: true,
      validateSchemas: true,
      validateReferenceConsistency: true,
      allowedHosts: ['registry.npmjs.org', 'npm.amorphie.com'],
   ...
   * @param {string} ref - Reference string like "@amorphie/domain-core/Tasks/task-invalidate-cache.1.0.0.json"
  ...
  parseRef(ref) {
    ...
    // External reference: "@amorphie/domain-core/Tasks/task-invalidate-cache.1.0.0.json"
  ...
  async resolveExternalRef(parsedRef) {
    ...
    // Load amorphie.config.json from the package
    const configPath = path.join(packagePath, 'amorphie.config.json');
    if (!(await fs.pathExists(configPath))) {
      throw new Error(`Package ${parsedRef.packageName} does not contain amorphie.config.json`);
    }
   ...
   * @param {Object} packageConfig - Package amorphie.config.json
  ...
  /**
   * Loads validation configuration from amorphie.config.json
   * @param {string} configPath - Path to amorphie.config.json
  ...
  async listPackageExports(packageName) {
    ...
    if (!(await fs.pathExists(configPath))) {
      throw new Error(`Package ${packageName} does not contain amorphie.config.json`);
    }
