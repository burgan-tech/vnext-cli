...
class TemplateManager {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      ...configDefaults,
      templateRepo: process.env.AMORPHIE_TEMPLATE_REPO || configDefaults.templateRepo || 'https://github.com/amorphie/amorphie-template.git',
  ...
  getSystemCacheDir() {
    ...
    switch (platform) {
      case 'darwin': // macOS
        return path.join(homedir, 'Library', 'Caches', 'amorphie-cli');
      
      case 'win32': // Windows
        return path.join(process.env.APPDATA || path.join(homedir, 'AppData', 'Roaming'), 'amorphie-cli', 'cache');
      
      case 'linux': // Linux
      default: // Other Unix-like systems
        return path.join(process.env.XDG_CACHE_HOME || path.join(homedir, '.cache'), 'amorphie-cli');
    }
  }
  ...
  async getTemplateInfo(version = 'latest') {
    try {
      ...
      const configPath = path.join(templatePath, 'amorphie.config.json');
